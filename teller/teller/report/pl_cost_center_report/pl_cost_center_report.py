...
def set_gl_entries_by_account(dimension_list, filters, account, gl_entries_by_account):
    ...
    if account:
        condition += " and account in ({})".format(
            ", ".join([frappe.db.escape(d) for d in account])
        )
...
def format_gl_entries(
    gl_entries_by_account, accounts_by_name, dimension_list, dimension_type
):
            ...
            for dimension in dimension_list:
                if dimension == entry.get(dimension_type):
                    d[frappe.scrub(dimension)] = (
                        d.get(frappe.scrub(dimension), 0.0)
                        + flt(entry.debit)
                        - flt(entry.credit)
                    )
...
#             "account_name": (
#                 "{} - {}".format(d.account_number, d.account_name)
#                 if d.account_number
#                 else d.account_name
#             ),
#         }
...
# def prepare_data(accounts, filters, company_currency, dimension_list):
#     data = []
#     profit_data = {}  # Dictionary to hold profit data for each dimension
...
#         for dimension in dimension_list:
#             if dimension not in profit_data:
#                 profit_data[dimension] = 0
#             profit_data[dimension] += flt(d.get(frappe.scrub(dimension), 0.0), 3)

#     # Append profit data as a separate row
#     profit_row = {
#         "account": "Profit",
#         "parent_account": "",
#         "indent": 0,
...
#         "account_name": "Profit",
#         "has_value": True,
#         "total": sum(profit_data.values()),  # Total profit across all dimensions
#     }
#     for dimension in dimension_list:
#         profit_row[frappe.scrub(dimension)] = profit_data[dimension]
...
def prepare_data(accounts, filters, company_currency, dimension_list):
    ...
    for d in accounts:
        ...
        for dimension in dimension_list:
            dimension_value = flt(d.get(frappe.scrub(dimension), 0.0), 3)
            row[frappe.scrub(dimension)] = dimension_value

            if abs(dimension_value) >= 0.005:
                has_value = True
                total += dimension_value

            # Classify and accumulate values into income or expense
            if d.root_type == "Income":
                if dimension not in income_data:
                    income_data[dimension] = 0
                income_data[dimension] += dimension_value
            elif d.root_type == "Expense":
                if dimension not in expense_data:
                    expense_data[dimension] = 0
                expense_data[dimension] += dimension_value

        row["has_value"] = has_value
        row["total"] = total
        data.append(row)

    # Calculate and append profit data as a separate row
    profit_row = {
        "account": "Profit",
        "parent_account": "",
        "indent": 0,
        ...
        "account_name": "Profit",
        "has_value": True,
        "total": sum(income_data.values()) - sum(expense_data.values()),  # Total profit
    }
    for dimension in dimension_list:
        profit_row[frappe.scrub(dimension)] = income_data.get(
            dimension, 0
        ) - expense_data.get(dimension, 0)
...
#     data = []
#     totals = {
#         dimension: 0 for dimension in dimension_list
...
#             dimension_value = flt(d.get(frappe.scrub(dimension), 0.0), 3)
#             row[frappe.scrub(dimension)] = dimension_value
#             row_total += dimension_value

#             # Accumulate totals by dimension based on root_type
#             if d.root_type in ["Income", "Expense"]:
#                 totals[dimension] += (
#                     dimension_value if d.root_type == "Income" else -dimension_value
#                 )
...
#     }
#     for dimension in dimension_list:
#         profit_row[frappe.scrub(dimension)] = totals[dimension]
...
def accumulate_values_into_parents(accounts, accounts_by_name, dimension_list):
    ...
    for d in reversed(accounts):
        if d.parent_account:
            for dimension in dimension_list:
                accounts_by_name[d.parent_account][
                    frappe.scrub(dimension)
                ] = accounts_by_name[d.parent_account].get(
                    frappe.scrub(dimension), 0.0
                ) + d.get(
                    frappe.scrub(dimension), 0.0
                )
