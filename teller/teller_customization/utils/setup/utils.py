...
def get_exchange_rate(from_currency, to_currency, transaction_date=None, args=None):
    if not (from_currency and to_currency):
        ...
        return  # Returning a tuple with default values
    if from_currency == to_currency:
        #  add return as tuble to handle to custom rates
        return (1,1)
    ...
    entries = frappe.get_all(
        "Currency Exchange",
        fields=["exchange_rate", "custom_selling_exchange_rate"],
        filters=filters,
        order_by="date desc",
        limit=1,
    )
    if entries:
        return flt(entries[0].exchange_rate), flt(entries[0].custom_selling_exchange_rate) 
