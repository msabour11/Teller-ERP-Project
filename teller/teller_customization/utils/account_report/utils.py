...
from teller.teller_customization.utils.setup.utils import get_exchange_rate
...
def get_currency(filters):
    ...
    if not report_date:
        fiscal_year_to_date = get_from_and_to_date(filters.get("to_fiscal_year"))[
            "to_date"
        ]
...
# def get_rate_as_at(date, from_currency, to_currency, voucher_type=None):
#     """
...
#     :param voucher_type: Type of the voucher, used for custom rates
#     :return: Retrieved exchange rate
#     """
#     rate_key = "{0}-{1}@{2}".format(from_currency, to_currency, date)
#     if voucher_type == "Teller Invoice":
#         # Fetch custom selling rate for Teller Invoice
#         rate = get_exchange_rate(from_currency, to_currency, date)[1]
#     else:
#         rate = __exchange_rates.get(rate_key)
#         if not rate:
#             rate = get_exchange_rate(from_currency, to_currency, date)[0] or 1
#             __exchange_rates[rate_key] = rate
...
#             converted_debit_value = convert(
#                 debit, presentation_currency, company_currency, date
#             )
#             converted_credit_value = convert(
#                 credit, presentation_currency, company_currency, date
#             )
...
def convert(value, from_, to, date, voucher_type=None):
    """
    Convert `value` from `from_` to `to` on `date`
    :param value: Amount to be converted
    :param from_: Currency of `value`
    ...
    :return: Result of converting `value`
    """
    rate = get_rate_as_at(date, from_, to, voucher_type)
    converted_value = flt(value) / (rate or 1)
    return converted_value
...
def convert_to_presentation_currency(gl_entries, currency_info):
    ...
    for entry in gl_entries:
        ...
        if len(account_currencies) == 1 and account_currency == presentation_currency:
            ...
            converted_debit_value = convert(debit, presentation_currency, company_currency, date, voucher_type)  
         
            converted_credit_value = convert(credit, presentation_currency, company_currency, date, voucher_type) 
            if entry.get("debit"):
                entry["debit"] = converted_debit_value
...
def get_party_details(party_type, party_list):
    ...
    query = (
        ...
        .select(party.name, party.tax_id)
        .where(party.name.isin(party_list))
    )
...
def get_journal_entries(filters, args):
    ...
    query = apply_common_conditions(
        filters, query, doctype="Journal Entry", payments=True
    )
...
def get_payment_entries(filters, args):
    ...
    query = apply_common_conditions(
        filters, query, doctype="Payment Entry", payments=True
    )
